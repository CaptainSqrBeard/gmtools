...
command.AssignSharedCommand("chardata",function (args, interface)
    ...
    if char == nil then
        interface.showMessage("GMTools: "..lang.Lang("Error_CharacterNotFound"),Color(255,0,128,255))
        return
    end
    ...
    if args[2] == nil then
        ...
        if cl ~= nil then
            ...
            interface.showMessage(lang.Lang("CMD_CharData_main_uncontrolled"),Color(255,255,255,255))
        end
        if char.Inventory ~= nil then
            interface.showMessage(lang.Lang("CMD_CharData_main_has_inv",{char.ID}),Color(255,255,255,255))
        end

        
    elseif args[2] == "see_inv" then
        if char.Inventory == nil then
            interface.showMessage("GMTools: "..lang.Lang("CMD_CharData_no_inv"),Color(255,0,0,255))
            return
        end
        ...
        for item in char.Inventory.AllItems do
            if item.OwnInventory ~= nil then
                ...
                interface.showMessage(lang.Lang("CMD_CharData_inv_item",{item.Prefab.Identifier.Value, item.ID}),Color(255,255,255,255))
            end
        end
    else
        interface.showMessage("GMTools: "..lang.Lang("CMD_CharData_UnknownInput"),Color(255,0,0,255))
    end
end)