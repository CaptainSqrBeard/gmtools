...
command.AssignClientCommand("humanlist",function(client,cursor,args)
    ...
    for i, char in ipairs(Character.CharacterList) do
        if char.IsHuman then
            utils.SendConsoleMessage(lang.Lang("CMD_HumanList_char",{char.Name, char.ID, gameInfo.GetLocalizedTeam(char.TeamID)}),client,Color(255,255,255,255))
        end
    end
end)
...
command.AssignServerCommand("humanlist",function(args)
    ...
    for i, char in ipairs(Character.CharacterList) do
        if char.IsHuman then
            utils.NewConsoleMessage(lang.Lang("CMD_HumanList_char",{char.Name, char.ID, gameInfo.GetLocalizedTeam(char.TeamID)}),Color(255,255,255,255))
        end
    end
end)