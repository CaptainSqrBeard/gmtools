...
local function sendAdminPMToPlayer(sender,recipient,msg)
    ...
    utils.SendConsoleMessage(GMT.Lang("CMD_AdminPM_con_for_player_L1",{sender.Name,recipient.Name}),recipient,Color(255,0,0,255))
    utils.SendConsoleMessage(GMT.Lang("CMD_AdminPM_con_for_player_L2",{msg}),recipient,Color(255,255,255,255))
    utils.SendConsoleMessage(GMT.Lang("CMD_AdminPM_con_for_player_L3"),recipient,Color(255,255,255,255))
end
...
local function sendConsolePMToPlayer(recipient,msg)
    ...
    utils.SendConsoleMessage(GMT.Lang("CMD_AdminPM_con_for_player_L1",{GMT.Lang("Console"),recipient.Name}),recipient,Color(255,0,0,255))
    utils.SendConsoleMessage(GMT.Lang("CMD_AdminPM_con_for_player_L2",{msg}),recipient,Color(255,255,255,255))
    utils.SendConsoleMessage(GMT.Lang("CMD_AdminPM_con_for_player_L3"),recipient,Color(255,255,255,255))
end

command.AddCommand("adminpm",GMT.Lang("Help_AdminPM"),false,nil,{
...
command.AssignClientCommand("adminpm", function(client,cursor,args)
    ...
    -- For sender
    utils.SendConsoleMessage(GMT.Lang("CMD_AdminPM_con_for_admin_L1",{client.Name,r_client.Name}),client,Color(255,0,0,255))
    utils.SendConsoleMessage(GMT.Lang("CMD_AdminPM_con_for_admin_L2",{msg}),client,Color(255,255,255,255))

    -- For other admins
    for i, cl in ipairs(Client.ClientList) do
        if (cl.SessionId ~= client.SessionId and cl.SessionId ~= r_client.SessionId) and GMT.HasPermission(cl,".adminpm") then
            utils.SendConsoleMessage(GMT.Lang("CMD_AdminPM_con_to_other_L1",{client.Name,r_client.Name}),cl,Color(255,0,0,255))
            utils.SendConsoleMessage(GMT.Lang("CMD_AdminPM_con_to_other_L2",{msg}),cl,Color(255,255,255,255))
        end
    end
...
command.AssignServerCommand("adminpm", function(args)
    ...
    if string.len(msg) > 200 then
        utils.NewConsoleMessage("GMTools: "..GMT.Lang("Error_TooLongMessage"),Color(255,0,0,255),false)
        return
    end
    msg = msg:sub(1, msg:len()-1)
    if msg:len() == 0 then
        utils.NewConsoleMessage("GMTools: "..GMT.Lang("CMD_AdminPM_NoMessage"),Color(255,0,0,255),false)
        return
    end

    -- For sender
    utils.NewConsoleMessage(GMT.Lang("CMD_AdminPM_con_for_admin_L1",{GMT.Lang("Console"),r_client.Name}),Color(255,0,0,255),false)
    utils.NewConsoleMessage(GMT.Lang("CMD_AdminPM_con_for_admin_L2",{msg}),Color(255,255,255,255),false)

    -- For other admins
    for i, cl in ipairs(Client.ClientList) do
        if GMT.HasPermission(cl,".adminpm") then
            utils.SendConsoleMessage(GMT.Lang("CMD_AdminPM_con_to_other_L1",{GMT.Lang("Console"),r_client.Name}),cl,Color(255,0,0,255))
            utils.SendConsoleMessage(GMT.Lang("CMD_AdminPM_con_to_other_L2",{msg}),cl,Color(255,255,255,255))
        end
    end
